...
function AbsenteeEntries({
    ...
    const [attendanceData, setAttendanceData] = useState(() =>
        rollNumbers.reduce((acc, rollNo) => {
            acc[rollNo] = { present: true, booklet: startBookletNo + (rollNo - (startNo + 1)) };
            return acc;
        }, {})
    );

    const handleSubmit = async () => {
        // Prepare the attendance array with updated booklet numbers
        ...
        // Pass the array to SendBackendReq
        const temp = await SendBackendReq(attendanceArray, startNo + 1, selectedSubject);
